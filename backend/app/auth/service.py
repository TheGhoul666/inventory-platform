...
class AuthService:
    ...
    async def get_roles(self, db: AsyncSession) -> list[dict]:
        """Returns system roles (code-defined) + custom roles (DB-stored, best-effort)."""
        result = []
        ...
        try:
            ...
            pass  # DB unavailable — return system roles only
        return result
    ...
    async def create_user(
        ...
        try:
            result = await asyncio.to_thread(
                self._admin.auth.admin.create_user,
                {
                    ...
                    "password": password,
                    "email_confirm": True,
                    "user_metadata": {"full_name": full_name, "username": username, "phone": phone},
                    "app_metadata": {
                        "roles": [role],
        ...
        return {"id": result.user.id, "email": result.user.email, "roles": [role], "phone": phone}

    async def update_user_profile(
        self, user_id: str, full_name: Optional[str] = None,
        username: Optional[str] = None, phone: Optional[str] = None,
    ) -> dict:
        """Update a user's profile metadata (full_name / username / phone).

        Phone is stored in Supabase Auth user_metadata.phone and is what the
        alert engine reads to send SMS notifications to SuperAdmins.
        """
        existing = await asyncio.to_thread(self._admin.auth.admin.get_user_by_id, user_id)
        meta: dict = {}
        if existing and existing.user:
            meta = dict(existing.user.user_metadata or {})
