...
# Hard ceiling for any single startup dependency connect. Startup must never
# block on an unreachable external service — degrade gracefully instead.
STARTUP_CONNECT_TIMEOUT = 8.0
...
async def lifespan(app: FastAPI):
    ...
    try:
        await asyncio.wait_for(redis_client.connect(), timeout=STARTUP_CONNECT_TIMEOUT)
    except (Exception, asyncio.TimeoutError) as exc:
        logger.warning("Redis unavailable — caching and rate-limiting disabled: %s", exc)

    try:
        await asyncio.wait_for(event_publisher.connect(), timeout=STARTUP_CONNECT_TIMEOUT)
    except (Exception, asyncio.TimeoutError) as exc:
        logger.warning("RabbitMQ unavailable — background workers disabled: %s", exc)

    try:
        await asyncio.wait_for(webhook_engine.connect(), timeout=STARTUP_CONNECT_TIMEOUT)
    except (Exception, asyncio.TimeoutError) as exc:
        logger.warning("Webhook engine init failed — webhook delivery disabled: %s", exc)
