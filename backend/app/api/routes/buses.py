...
async def create_bus(
    ...
    await write_audit_log(
        ...
        "CREATE", "buses", bus_id,
        notes=f"Created bus fleet_number={body.fleet_number} plate={body.license_plate}",
        new_values={"fleet_number": body.fleet_number, "license_plate": body.license_plate, "status": body.status},
    )
...
async def update_bus(
    ...
    await write_audit_log(
        sb, str(current_user.id), current_user.username or current_user.email,
        "UPDATE", "buses", str(bus_id),
        notes=f"Updated bus fields: {list(updates.keys())}",
        new_values=updates,
    )
    return BusResponse.model_validate(result.data[0])
