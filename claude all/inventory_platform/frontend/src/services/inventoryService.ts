...
export const usersService = {
  listUsers: async (page = 1, perPage = 50): Promise<{ users: any[] }> => {
    const { data } = await api.get("/auth/users", { params: { page, per_page: perPage } });
    return data;
  },
  ...
  updateUserPermissions: async (userId: string, add: string[], remove: string[]) => {
    const { data } = await api.patch(`/auth/users/${userId}/permissions`, { add, remove });
    return data;
  },
...
export const rolesService = {
  listRoles: async (): Promise<{ roles: any[]; all_permissions: string[] }> => {
  ...
  createRole: async (payload: {
    ...
    const { data } = await api.post("/auth/roles", payload);
    return data;
  },

  deleteRole: async (name: string) => {
    await api.delete(`/auth/roles/${name}`);
  },
};
