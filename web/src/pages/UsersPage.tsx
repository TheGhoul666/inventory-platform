...
import { Plus, Trash2, X, Users, KeyRound, Check, Phone } from "lucide-react";
import toast from "react-hot-toast";
import { usersService, rolesService } from "@/services/inventoryService";
...
function InviteUserModal({ onClose }: { onClose: () => void }) {
  ...
  const isPhoneValid = !form.phone || PHONE_REGEX.test(form.phone);
  const valid = form.email && form.full_name && form.username && form.password.length >= 8 && isPhoneValid;
  ...
  return (
    <Modal title="Invite User" onClose={onClose}>
      <div className="space-y-3">
        ...
        <Field label="Phone (SMS alerts — e.g. +972501234567)">
          <input
            value={form.phone}
            onChange={set("phone")}
          ...
          {form.phone && !isPhoneValid && (
            <p className="text-xs text-red-400 mt-1">Invalid format. Use +972501234567 or similar.</p>
          )}
        </Field>
        <Field label="Role">
...
function EditPermissionsModal({
  ...
  const qc = useQueryClient();
  const rolePerms: string[] = user.permissions ?? [];
  const extra: string[] = user.extra_permissions ?? [];
  const [extraSet, setExtraSet] = useState<Set<string>>(new Set(extra));

  const toggle = (p: string) => {
    if (rolePerms.includes(p)) return; // role permissions are read-only
    setExtraSet((s) => {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      const add = [...extraSet].filter((p) => !extra.includes(p));
      const remove = extra.filter((p) => !extraSet.has(p));
      return usersService.updateUserPermissions(user.id, add, remove);
    },
    onSuccess: () => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface-800 rounded-xl border border-surface-700 p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-lg font-semibold text-white">הרשאות: {user.email}</h3>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-300">
            <X className="h-5 w-5" />
          </button>
        </div>
        <p className="text-xs text-slate-500 mb-4">
          הרשאות מתפקיד ({user.roles?.[0]}) מסומנות בכחול ואינן ניתנות לשינוי.
          ניתן להוסיף / להסיר הרשאות נוספות מעבר לתפקיד.
        </p>

        <div className="grid grid-cols-2 gap-1.5 max-h-60 overflow-y-auto mb-5 pr-1">
          {allPermissions.map((p) => {
            const fromRole = rolePerms.includes(p);
            const checked = fromRole || extraSet.has(p);
            return (
              <button
                key={p}
                onClick={() => toggle(p)}
                ...
                className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs text-left transition-colors ${
                  fromRole
                    ? "bg-brand-900/40 text-brand-300 cursor-default"
                    : checked
              ...
              >
                <div className={`h-3.5 w-3.5 rounded border flex items-center justify-center shrink-0 ${
                  checked ? (fromRole ? "bg-brand-500 border-brand-500" : "bg-green-500 border-green-500") : "border-slate-500"
                }`}>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 px-4 py-2 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors"
          >
...
const isValidPhone = (value: string) => !value || PHONE_REGEX.test(value);

function EditPhoneModal({ user, onClose }: { user: any; onClose: () => void }) {
  const qc = useQueryClient();
  const [phone, setPhone] = useState<string>(user.phone ?? "");
  const [phoneError, setPhoneError] = useState("");

  const mutation = useMutation({
    mutationFn: () => usersService.updateUserProfile(user.id, { phone }),
    onSuccess: () => {
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value && !isValidPhone(value)) {
      setPhoneError("Invalid format. Use +972501234567 or similar.");
    } else {
      setPhoneError("");
    }
  };
  ...
  return (
    <Modal title={`Phone — ${user.email}`} onClose={onClose}>
      <div className="space-y-3">
        <p className="text-xs text-slate-500">
          SuperAdmins receive SMS alerts at this number for stock shortages and
          significant inventory changes. Use international format (e.g. +972501234567).
        </p>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button
          onClick={() => mutation.mutate()}
          disabled={mutation.isPending || !isValidPhone(phone)}
          className="flex-1 px-4 py-2 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors"
        >
...
export function UsersPage() {
  ...
  const allPermissions: string[] = rolesData?.all_permissions ?? [];
  ...
  return (
    <div className="space-y-4">
      ...
      {editPhoneUser && (
        <EditPhoneModal user={editPhoneUser} onClose={() => setEditPhoneUser(null)} />
      )}
                    ...
                    <tr key={user.id} className="hover:bg-surface-700/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => setEditPhoneUser(user)}
                            className="p-1.5 rounded text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 transition-colors"
                            title="Edit phone (SMS alerts)"
                          >
                            <Phone className="h-4 w-4" />
                          </button>
                          <button
                            onClick={() => setEditPermUser(user)}
                            className="p-1.5 rounded text-slate-400 hover:text-brand-400 hover:bg-brand-500/10 transition-colors"
                            title="עריכת הרשאות"
                          >
